...
- **Rich path expressions** — wildcards, filters with `&&`/`||`, recursive descent, slices (`[0:5:2]`), multi-index (`[0,2,4]`), multi-key picks (`[name,age]`), and filter functions (`starts_with`, `contains`, `values`)
- **Security-first** — prototype pollution guard · XML XXE prevention (DOCTYPE/ENTITY blocked) · static ReDoS analysis on regex filters · configurable `SecurityPolicy`
- **Plugin system** — override parsers/serializers per format via `PluginRegistry`
...
- **CLI** — query, transform, and convert files from the terminal
- **Auto-detected extensions** — `.json`, `.yml`, `.yaml`, `.toml`, `.ini`, `.cfg`, `.env`, `.ndjson`, `.jsonl`
