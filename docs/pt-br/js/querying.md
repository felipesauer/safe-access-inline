...
```typescript
const template = "users.{id}.profile.{field}";

const name = accessor.getTemplate(template, { id: "0", field: "name" });
...
for (const field of fields) {
    profile[field] = accessor.getTemplate("users.{id}.{field}", {
        id: "0",
        field,
    });
}
...
Leia vários paths de uma só vez, recebendo um mapa de resultados:
